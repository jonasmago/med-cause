...
df.mc_fitted_V2 <- data.frame(matrix("", nrow = length (df.subject_V2[,1]), ncol = 86))
for (i in 1:86){
  df.mc_fitted_V2[,i] = as.numeric(df.mc_fitted_V2[,i])
}

colnames (df.mc_fitted_V2) <- c("baseline_MLE",
                             "baseline_BIC", 
                             "PID_softmax",
                             "PID_lazyness", 
                             "PID_stickiness",
...
#baseline
###########################
df.mc_fitted_V2[,1] = log (1/3)*4000
df.mc_fitted_V2[,2] = -2*df.mc_fitted_V2[,1] + log(4000)
...
for (k in model_selection){
  ...
  for (j in participant_selection){ #participants
    ...
    df.mc_fitted_V2[j,(3+k*7):(5+k*7)] = t(data.frame(opt_param[1]))
    df.mc_fitted_V2[j,(8+k*7)] = opt_param[2] #MLE
    
    #model 2 // softmaz and lazyness
    opt_param = optim((c(start_softmax,start_lazy_sticky)), fn_02, control = list(fnscale=fnscale))
    df.mc_fitted_V2[j,(3+k*7+21):(4+k*7+21)] = t(data.frame(opt_param[1]))
    df.mc_fitted_V2[j,(8+k*7+21)] = opt_param[2] #MLE
    
    #model 3 // softmax and stickyness 
    opt_param = optim((c(start_softmax,start_lazy_sticky)), fn_03, control = list(fnscale=fnscale))
    df.mc_fitted_V2[j,c((3+k*7+42),(5+k*7+42))] = t(data.frame(opt_param[1]))
    df.mc_fitted_V2[j,(8+k*7+42)] = opt_param[2] #MLE
    
    #model 4 // only softmax
    opt_param = optim((c(start_softmax)), fn_04, method="Brent", lower=0, upper=1000, control = list(fnscale=fnscale))
    df.mc_fitted_V2[j,(3+k*7+63)] = t(data.frame(opt_param[1]))
    df.mc_fitted_V2[j,(8+k*7+63)] = opt_param[2] #MLE
    
    
  }
  for (m in 0:3){
    df.mc_fitted_V2[,(6+k*7+21*m):(7+k*7+21*m)] = logit (df.mc_fitted_V2[,(4+k*7+21*m):(5+k*7+21*m)])
  }
}
...
####################################
for (k in 0:2){
  m= 0; pun = 3; df.mc_fitted_V2[,(9+k*7+21*m)] = -2*df.mc_fitted_V2[,(8+k*7+21*m)]+3*log(4000)
  m= 1; pun = 2; df.mc_fitted_V2[,(9+k*7+21*m)] = -2*df.mc_fitted_V2[,(8+k*7+21*m)]+pun*log(4000)
  m= 2; pun = 2; df.mc_fitted_V2[,(9+k*7+21*m)] = -2*df.mc_fitted_V2[,(8+k*7+21*m)]+pun*log(4000)
  m= 3; pun = 1; df.mc_fitted_V2[,(9+k*7+21*m)] = -2*df.mc_fitted_V2[,(8+k*7+21*m)]+pun*log(4000)
}

'''
for (k in 0:2){
  m= 0; pun = 3; df.mc_fitted_V2[,(9+k*7+21*m)] = -2*df.mc_fitted_V2[,(8+k*7+21*m)]+3*log(4000)
  m= 1; pun = 3; df.mc_fitted_V2[,(9+k*7+21*m)] = -2*df.mc_fitted_V2[,(8+k*7+21*m)]+pun*log(4000)
  m= 2; pun = 3; df.mc_fitted_V2[,(9+k*7+21*m)] = -2*df.mc_fitted_V2[,(8+k*7+21*m)]+pun*log(4000)
  m= 3; pun = 3; df.mc_fitted_V2[,(9+k*7+21*m)] = -2*df.mc_fitted_V2[,(8+k*7+21*m)]+pun*log(4000)
}
'''