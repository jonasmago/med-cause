### REVERSE: df.bayes_opt_response_NEW
### df.mc_fitted_combined_backup
...
df.mc_likelihood <- data.frame(matrix("", nrow = (length(df.results[,2])), ncol = 12))
...
df.mc_likelihood[,1] = as.data.frame(ifelse (df.results[,3]==1, 1, 0))
df.mc_likelihood[,2]= as.data.frame(ifelse (df.results[,3]==3, 1, 0))
df.mc_likelihood[,3] = as.data.frame(ifelse (df.results[,3]==0, 1, 0))
df.mc_likelihood[,4] = ifelse ((df.results[,2]-df.results[,5]) < 0, 1,0) #below (press up)
df.mc_likelihood[,5] = ifelse ((df.results[,2]-df.results[,5]) > 0, 1,0) #above (press down)
df.mc_likelihood[,6] = ifelse ((df.results[,2]-df.results[,5]) == 0, 1,0)
...
df.mc_likelihood[,10] = as.data.frame(df.bayes[,5] * df.bayes_opt_response_NEW[,1]+df.bayes[,6] * df.bayes_opt_response_NEW[,1]+df.bayes[,7] * df.bayes_opt_response_NEW[,4]+df.bayes[,8] * df.bayes_opt_response_NEW[,4])
df.mc_likelihood[,11] = as.data.frame(df.bayes[,5] * df.bayes_opt_response_NEW[,2]+df.bayes[,6] * df.bayes_opt_response_NEW[,2]+df.bayes[,7] * df.bayes_opt_response_NEW[,5]+df.bayes[,8] * df.bayes_opt_response_NEW[,5])
df.mc_likelihood[,12] = as.data.frame(df.bayes[,5] * df.bayes_opt_response_NEW[,3]+df.bayes[,6] * df.bayes_opt_response_NEW[,3]+df.bayes[,7] * df.bayes_opt_response_NEW[,6]+df.bayes[,8] * df.bayes_opt_response_NEW[,6])
...
df.mc_fitted_combined <- data.frame(matrix("", nrow = 1, ncol = 86))
for (i in 1:86){
  df.mc_fitted_combined[,i] = as.numeric(df.mc_fitted_combined[,i])
}

colnames (df.mc_fitted_combined) <- c("baseline_MLE",
                             "baseline_BIC", 
                             "PID_softmax",
                             "PID_lazyness", 
                             "PID_stickiness",
...
#baseline
###########################
df.mc_fitted_combined[,1] = log (1/3)*4000*44
df.mc_fitted_combined[,2] = -2*df.mc_fitted_combined[,1] + log(4000)
...
for (k in model_selection){
  ...
  for (j in 1:1){ #participants
    ...
    df.mc_fitted_combined[j,(3+k*7):(5+k*7)] = t(data.frame(opt_param[1]))
    df.mc_fitted_combined[j,(8+k*7)] = opt_param[2] #MLE
    print ("model 1 complete")
    
    #model 2 // softmaz and lazyness
    opt_param = optim((c(start_softmax,start_lazy_sticky)), fn_02, control = list(fnscale=fnscale))
    df.mc_fitted_combined[j,(3+k*7+21):(4+k*7+21)] = t(data.frame(opt_param[1]))
    df.mc_fitted_combined[j,(8+k*7+21)] = opt_param[2] #MLE
    print ("model 2 complete")
    
    #model 3 // softmax and stickyness 
    opt_param = optim((c(start_softmax,start_lazy_sticky)), fn_03, control = list(fnscale=fnscale))
    df.mc_fitted_combined[j,c((3+k*7+42),(5+k*7+42))] = t(data.frame(opt_param[1]))
    df.mc_fitted_combined[j,(8+k*7+42)] = opt_param[2] #MLE
    ...
    df.mc_fitted_combined[j,(3+k*7+63)] = t(data.frame(opt_param[1]))
    df.mc_fitted_combined[j,(8+k*7+63)] = opt_param[2] #MLE
    print ("model 4 complete")
    
    
  }
  for (m in 0:3){
    df.mc_fitted_combined[,(6+k*7+21*m):(7+k*7+21*m)] = logit (df.mc_fitted_combined[,(4+k*7+21*m):(5+k*7+21*m)])
  }
}
...
####################################
for (k in 0:2){
  m= 0; pun = 3; df.mc_fitted_combined[,(9+k*7+21*m)] = -2*df.mc_fitted_combined[,(8+k*7+21*m)]+3*log(4000)
  m= 1; pun = 2; df.mc_fitted_combined[,(9+k*7+21*m)] = -2*df.mc_fitted_combined[,(8+k*7+21*m)]+pun*log(4000)
  m= 2; pun = 2; df.mc_fitted_combined[,(9+k*7+21*m)] = -2*df.mc_fitted_combined[,(8+k*7+21*m)]+pun*log(4000)
  m= 3; pun = 1; df.mc_fitted_combined[,(9+k*7+21*m)] = -2*df.mc_fitted_combined[,(8+k*7+21*m)]+pun*log(4000)
}
